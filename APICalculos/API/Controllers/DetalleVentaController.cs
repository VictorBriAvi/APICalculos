    ...
    public class DetalleVentaController : ControllerBase
    {
        ...
        public async Task<ActionResult<DetalleVenta>> BuscarDetalleVentaId(int id)
        {
            ...
            var detalleVentaDTO = new DetalleVentaDTO
            {
                ...
                NombreClienteVenta = detalleVentaId.Venta.Client.Name ,
                NombreTipoDeServicioVenta = detalleVentaId.TipoDeServicio.Name,
                PrecioTipoDeServicio = detalleVentaId.TipoDeServicio.Price,
                NombreEmpleadoVenta = detalleVentaId.Empleado.Name,
            };
        ...
        public async Task<ActionResult<IEnumerable<DetalleVentaDTO>>> BuscarDetallesPorVentaId(int ventaId)
        {
            ...
            var detallesVentaDTO = detallesVenta.Select(detalle => new DetalleVentaDTO
            {
                ...
                NombreClienteVenta = detalle.Venta.Client.Name,
                NombreTipoDeServicioVenta = detalle.TipoDeServicio.Name,
                PrecioTipoDeServicio = detalle.TipoDeServicio.Price,
                NombreEmpleadoVenta = detalle.Empleado.Name,
            }).ToList();
        ...
        [HttpPost]
        public async Task<ActionResult> AgregarDetallesVenta(List<DetalleVentaCreacionDTO> detallesVenta)
        {
            foreach (var detalle in detallesVenta)
            {
                var venta = await _context.Ventas.FindAsync(detalle.VentaId);
                var tipoDeServicio = await _context.ServiceTypes.FindAsync(detalle.TipoDeServicioId);
                var empleado = await _context.Employees.FindAsync(detalle.EmpleadoId);

                if (venta == null)
                    return BadRequest($"La venta con ID {detalle.VentaId} no existe");

                if (tipoDeServicio == null)
                    return BadRequest($"El tipo de servicio con ID {detalle.TipoDeServicioId} no existe");

                if (empleado == null)
                    return BadRequest($"El empleado con ID {detalle.EmpleadoId} no existe");

                // 🔁 Mapea correctamente al tipo de entidad
                var detalleVenta = _mapper.Map<DetalleVenta>(detalle); // 👈 Aquí el cambio
                _context.Add(detalleVenta);
            }

            await _context.SaveChangesAsync();

            return Ok("Detalles guardados correctamente");
        }
